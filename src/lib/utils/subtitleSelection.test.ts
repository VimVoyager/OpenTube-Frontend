...
describe('Subtitle Selection Integration', () => {
	describe('complete workflow', () => {
		it('should select and log subtitles correctly', () => {
			vi.spyOn(console, 'log').mockImplementation(() => {});
			
			const input = createMockSubtitlesWithAutoGenerated();
			
			const selected = selectSubtitles(input);
			logSelectedSubtitles(selected);
			
			// Should deduplicate and prefer manual
			expect(selected.length).toBe(2);
			expect(selected.every(s => !s.autogenerated)).toBe(true);
			
			// Should log results
			expect(console.log).toHaveBeenCalledWith(
				expect.stringContaining('Selected 2 subtitle tracks')
			);
		});

		it('should handle format filtering + deduplication + sorting', () => {
			const input = [
				// VTT English - manual (should be selected, first)
				createMockSubtitle({
					locale: 'en',
					displayLanguageName: 'English',
					format: 'vtt',
				...
				// VTT English - auto (should be filtered by deduplication)
				createMockSubtitle({
					locale: 'en',
					displayLanguageName: 'English (auto)',
					format: 'vtt',
				...
				// VTT Spanish (should be selected, second)
				createMockSubtitle({
					locale: 'es',
					displayLanguageName: 'Spanish',
					format: 'vtt',
				...
				// TTML French (should be filtered by format)
				createMockSubtitle({
					locale: 'fr',
					displayLanguageName: 'French',
					format: 'ttml',
			...
			const result = selectSubtitles(input);
			
			// Format filter: only VTT
			expect(result.every(s => s.format === 'vtt')).toBe(true);
			
			// Deduplication: one per language
			expect(result.length).toBe(2);
			
			// Sorting: English first
			expect(result[0].locale).toBe('en');
			expect(result[1].locale).toBe('es');
			
			// Quality: prefer manual
			expect(result[0].autogenerated).toBe(false);
		});
	});

	describe('real-world scenarios', () => {
		it('should process typical YouTube subtitle list', () => {
			...
			const result = selectSubtitles(youtubeSubtitles);
			
			// Correct count
			expect(result.length).toBe(4);
			
			// Correct order: und -> en -> es -> fr
			expect(result.map(s => s.locale)).toEqual(['und', 'en', 'es', 'fr']);
			
			// English should be manual (deduplicated)
			const english = result.find(s => s.locale === 'en');
			expect(english?.autogenerated).toBe(false);
		});
	});
