...
describe('getManifest', () => {
	describe('successful manifest requests', () => {
		it('should return ManifestResponse object with all properties', async () => {
			...
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT10M30S'), { format: 'xml' });
		...
		it('should log manifest load information', async () => {
			...
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT5M0S'), { format: 'xml' });
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('duration=300s'));
			expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('test-video-id'));
	...
	describe('duration parsing', () => {
		...
		it('should parse duration with only seconds', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT45S'), { format: 'xml' });
		...
		it('should parse duration with only minutes', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT5M'), { format: 'xml' });
		...
		it('should parse duration with hours, minutes, and seconds', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT1H30M45S'), {
				format: 'xml'
			});
		...
		it('should parse duration with only hours', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT2H'), { format: 'xml' });
		...
		it('should parse duration with hours and seconds (no minutes)', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT1H30S'), { format: 'xml' });
		...
		it('should parse duration with decimal seconds', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT1M30.5S'), {
				format: 'xml'
			});
		...
		it('should return 0 duration when duration format is invalid', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('INVALID'), { format: 'xml' });
		...
		it('should handle zero duration', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT0S'), { format: 'xml' });
		...
		it('should handle very long durations', async () => {
			const videoId = 'test-id';
			const mockFetch = createSuccessfulFetch(createMockManifestXml('PT10H30M45S'), {
				format: 'xml'
			});
	...
	describe('HTTP error handling', () => {
		...
		it('should include video ID in error message', async () => {
			...
			await expect(getManifest(videoId, mockFetch)).rejects.toThrow(/error-test-id/);
		});
		...
		it('should include status code in error message', async () => {
			...
			await expect(getManifest(videoId, mockFetch)).rejects.toThrow(/503/);
		});
		...
		it('should include status text in error message', async () => {
			...
			await expect(getManifest(videoId, mockFetch)).rejects.toThrow(/Too Many Requests/);
		});
	...
	describe('network error handling', () => {
		it('should throw error on network failure', async () => {
			...
			await expect(getManifest(videoId, mockFetch)).rejects.toThrow('Failed to fetch');
		});
		...
		it('should throw error on timeout', async () => {
			...
			await expect(getManifest(videoId, mockFetch)).rejects.toThrow('Request timeout');
		});
		...
		it('should throw error on connection refused', () => {
			...
			expect(getManifest(videoId, mockFetch)).rejects.toThrow('Connection refused');
		});
	});
	...
	describe('edge cases', () => {
		...
		it('should handle concurrent requests independently', async () => {
			...
			const mockFetch1 = createSuccessfulFetch(createMockManifestXml('PT1M0S'), { format: 'xml' });
			const mockFetch2 = createSuccessfulFetch(createMockManifestXml('PT2M0S'), { format: 'xml' });
...
describe('getManifestUrl', () => {
	describe('backward compatibility', () => {
		...
		it('should return same URL as getManifest().url', async () => {
			...
			const urlResult = await getManifestUrl(
				videoId,
		...
		it('should handle network errors', async () => {
			...
			await expect(getManifestUrl(videoId, mockFetch)).rejects.toThrow('Network error');
		});
	});
